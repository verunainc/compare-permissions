...
(async () => {
    ...
    for (let targetCnt = 0; targetCnt < global.targetNames.length; targetCnt++) {
        ...
        const defaultOutputMap = new Map([
            [OBJECT_PERMISSION, [OBJECT_PERMISSION, objectPermissionMap, BOOLEAN_NO_OUTPUT, "CRUD", "R", "", true, false]],
            [FIELD_LEVEL_SECURITY, [FIELD_LEVEL_SECURITY, fieldLevelSecurityMap, BOOLEAN_NO_OUTPUT, "RU", "R", "", true, false]],
            [LAYOUT_ASSIGNMENT, [LAYOUT_ASSIGNMENT, layoutAssignmentMap, BOOLEAN_OUTPUT, METADATA_TRUE, "", METADATA_FALSE, false, true]],
            [RECORD_TYPE_VISIBILITY, [RECORD_TYPE_VISIBILITY, recordTypeVisibilityMap, BOOLEAN_NO_OUTPUT, appConfig.recordTypeVisibilityLabel.visible, "", "", true, false]],
            [APEX_CLASS_ACCESS, [APEX_CLASS_ACCESS, apexClassAccessMap, BOOLEAN_OUTPUT_WITH_NA, METADATA_TRUE, "", METADATA_FALSE, false, false]],
            [APEX_PAGE_ACCESS, [APEX_PAGE_ACCESS, apexPageAccessMap, BOOLEAN_OUTPUT_WITH_NA, METADATA_TRUE, "", METADATA_FALSE, false, false]],
            [USER_PERMISSION, [USER_PERMISSION, userPermissionMap, BOOLEAN_OUTPUT, METADATA_TRUE, "", METADATA_FALSE, false, false]],
            [APPLICATION_VISIBILITY, [APPLICATION_VISIBILITY, applicationVisibilityMap, BOOLEAN_NO_OUTPUT, appConfig.applicationVisibilityLabel.visible, "", "", true, false]],
            [
                TAB_VISIBILITY,
                [
                    TAB_VISIBILITY,
            ...
            [LOGIN_IP_RANGE, [LOGIN_IP_RANGE, loginIpRangeMap, BOOLEAN_OUTPUT, METADATA_TRUE, "", METADATA_FALSE, false, true]],
            [LOGIN_HOUR, [LOGIN_HOUR, loginHourMap, BOOLEAN_OUTPUT, METADATA_TRUE, "", METADATA_FALSE, false, true]],
            [CUSTOM_PERMISSION, [CUSTOM_PERMISSION, customPermissionMap, BOOLEAN_OUTPUT, METADATA_TRUE, "", METADATA_FALSE, false, false]],
            [CUSTOM_METADATA_TYPE_ACCESS, [CUSTOM_METADATA_TYPE_ACCESS, customMetadataTypeAccessMap, BOOLEAN_OUTPUT, METADATA_TRUE, "", METADATA_FALSE, false, false]],
            [CUSTOM_SETTING_ACCESS, [CUSTOM_SETTING_ACCESS, customSettingAccessMap, BOOLEAN_OUTPUT, METADATA_TRUE, "", METADATA_FALSE, false, false]],
            [SESSION_SETTING, [SESSION_SETTING, sessionSettingMap, BOOLEAN_NO_OUTPUT, "", "", "", false, true]],
            [PASSWORD_POLICY, [PASSWORD_POLICY, passwordPolicyMap, BOOLEAN_NO_OUTPUT, "", "", "", false, true]],
        ]);
...
function retrieveObjectPermissions(metadatas, orgName, isProfile, objectPermissionMap) {
    ...
    for (const metadata of metadatas) {
        ...
        objectPermissions.forEach(function (objectPermission) {
            ...
            if (targetObjectSet.size > 0 && !targetObjectSet.has(objectPermission.object)) {
                return;
            }
...
function retrievefieldLevelSecurities(metadatas, orgName, isProfile, fieldLevelSecurityMap, fieldLevelSecurityFieldSet) {
    ...
    for (const metadata of metadatas) {
        ...
        fieldPermissions.forEach(function (fieldPermission) {
            ...
            if (targetObjectSet.size > 0 && !targetObjectSet.has(fieldPermission.field.split(".")[0])) {
                return;
            }
...
function retrieveRecordTypeVisibilities(metadatas, orgName, isProfile, recordTypeVisibilityMap) {
    ...
    for (const metadata of metadatas) {
        ...
        recordTypeVisibilities.forEach(function (recordTypeVisibility) {
            ...
            if ((/^Account\./.exec(recordType) || /^Contact\./.exec(recordType)) && isTrue(recordTypeVisibility.personAccountDefault)) {
...
async function getPermissionSetAPINames(conn) {
    ...
    const records = [];
    await conn
        .query("SELECT Id, Name, Label, NamespacePrefix FROM PermissionSet WHERE Label IN " + condition)
...
async function compensateRecordTypeVisibilities(conn, recordTypeVisibilityMap) {
    if (isExecutable(RECORD_TYPE_VISIBILITY)) {
        ...
        const records = [];
        await conn
            .query("SELECT Name, SobjectType, DeveloperName FROM RecordType")
            .on("record", function (record) {
                records.push(record);
            })
            .on("error", function (err) {
...
async function compensateApexPageAccesses(conn, apexPageAccessMap) {
    if (isExecutable(APEX_PAGE_ACCESS)) {
        ...
        const records = [];
        await conn
            .query("SELECT Name, MasterLabel FROM ApexPage")
            .on("record", function (record) {
                records.push(record);
            })
            .on("error", function (err) {
...
async function compensateApplicationVisibilities(conn, applicationVisibilityMap) {
    if (isExecutable(APPLICATION_VISIBILITY)) {
        ...
        const records = [];
        await conn
            .query("SELECT NamespacePrefix, DeveloperName, Label FROM AppDefinition")
            .on("record", function (record) {
                records.push(record);
            })
            .on("error", function (err) {
...
async function compensateCustomPermissions(conn, customPermissionMap) {
    if (isExecutable(CUSTOM_PERMISSION)) {
        ...
        const records = [];
        await conn
            .query("SELECT NamespacePrefix, DeveloperName, MasterLabel FROM CustomPermission")
            .on("record", function (record) {
                records.push(record);
            })
            .on("error", function (err) {
...
async function compensateByEntityDefinition(conn, targetMap, metadataType) {
    if ((metadataType === CUSTOM_METADATA_TYPE_ACCESS && isExecutable(CUSTOM_METADATA_TYPE_ACCESS)) || (metadataType === CUSTOM_SETTING_ACCESS && isExecutable(CUSTOM_SETTING_ACCESS))) {
        ...
        const qualifiedApiNames = Array.from(targetMap.keys())
            .map((target) => {
                return "'" + target + "'";
            })
            .join(",");
        await conn.tooling
            .query("SELECT NamespacePrefix, QualifiedApiName, Label FROM EntityDefinition WHERE QualifiedApiName IN (" + qualifiedApiNames + ")")
            .on("record", function (record) {
                records.push(record);
            })
            .on("error", function (err) {
...
function loadUserConfig(userConfigPath) {
    ...
    for (const org of config.org) {
        const orgInfo = new _orgInfo(org.name, org.loginUrl, org.apiVersion, org.userName, org.password);
        orgs.push(orgInfo);
    }
    global.orgs = orgs;
}
